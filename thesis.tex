...
\begin{proof}
  ...
  \begin{itemize}
    \item [\ruleref{parL}] In this case, $P = \send{c}{a}; P'$. 
    If $c$ is an external channel, then all of $\config$ is final.
    Otherwise, $P$ provides $a$ and ends in $\pare L^*$ rule. The typing derivation of $\Phi \vdash \procObj{\Gamma}{\Delta}{\Omega}{\send{c}{a}; P'} :: \Phi'$ looks like
    \[
    ...
    By \stepref{channel-reduction-l}{channel-l}, we know that the configuration can reduce to another configuration without changing the set $\cancelSet$.
