...
\section{Reduction}
In future discussions, we'll distinguish channels allocated using $a,b,c,\ldots$ for newly created channels 
and use $x,y,z,\ldots$ for channel variables that require substitution by another allocated channel. 
...
and there are no remaining processes, which is formalized by the \stepref{finish}{end} rule.
\begin{figure}[H]
  \begin{alignat}{4}
...
We implement the idea of non-exhaustive matching in our system by raising an exception when a label does not have its corresponding branch.
