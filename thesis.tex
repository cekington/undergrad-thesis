...
We also formalize recursion to make program more expressive.
After that, we prove the safety theorems of the system by showing well typed configuration will not get stuck, 
...
The prefix of a process is the channel that the process is directly communicate to. We list the prefix of a process in the following:
\begin{align*}
...
We provide a wrapper process \texttt{wrap\_subtract} to call the \texttt{subtract} process,
since we do not allow exceptions which do not have any handling process. Because the process is for example usage,
we do not do anything interesting in the \texttt{catch} branch. We simply return a $0$ from another external channel \texttt{n\_exn}.

Information can also be exchanged between the main process and the exception handling process by creating a new channel.
For instance, when zipping two lists and the length of the lists are not equal, 
the first attempt of zip will raise an exception and passes the difference of the length of the lists to the second attempt, which is the exception process.
The second attempt can then remove the difference of the length of the lists and zip the rest of the lists again.
\chapter{Other features}
In this chapter, we explore some advanced features or remove some constraints in the system 
to make the language more capable in various situation. We study uncaught exception and non-exhaustive match in our system.
To adjust with the changes, we revisit the typing judgement and the reduction rules.
Moreover, we restate the safety theorem to coherent with the new system.
...
\begin{figure}[H]
  ...
  \label{fig:abortcfg}
\end{figure}
The typing judgement for the error configuration does not matter. 
Therefore, $\Lambda$ is well-typed for any input context $\Phi$ and output context $\Phi'$.
We add a new rule \stepref{uncaught}{uncaught}, which is another case of \stepref{activation}{activate} rule where $\Omega$ is empty.
...
\section{Non-exhaustive match}
Pattern match are used for casing on algebraic data types in functional programming languages. The label message reduction 
in our system looks like pattern match. For instance, when we are receiving messages from a channel of type \texttt{nat},
we need to provide a branch for each label in the type definition. Right now we require that every label must have its own branch 
as restricted by the \ruleref{orL} and \ruleref{withR}.
Non-exhaustive match is a common warning or error in most programming languages.
Such matching will cease the program execution if the expression does not match with any of the branches.
A good programming habit is to use a wild case to deal with all the remaining, possibly uninteresting cases.
We implement the idea of non-exhaustive match in our system by raise an exception when the label does not have its corresponding branch.
To relax such constraints, we modify the \ruleref{orL} and \ruleref{withR} rules in \cref{fig:nonexhaustive}.
The branching label collection $L'$ is a subset of the original label collection $L$, which means in the original old rule, $L' = L$.
\begin{figure}[H]
  \begin{rules}
    \defrule[$\oplus L'$][orL']{
  ...
  \end{rules}
  \caption{Refined typing judgement for non-exhaustive match}
  \label{fig:nonexhaustive}
\end{figure}
The original reduction rules for labels no longer holds. In \stepref{label-reduction-l}{label-l} 
and \stepref{label-reduction-r}{label-r} rule, we need to ensure that $k \in L$.
To account for the possibility of $k \not\in L$, we add two new reduction rules in \cref{fig:nonexreduction}.
The intended behavior is to raise an exception and channel all the channels that are prepared for the \textbf{recv} process.
\begin{figure}[H]
  \begin{alignat}{4}
    \label{label-reduction-r'} \textsc{[label-r']} & ~ 
      \Braket{
...
Channel cancellation is first proposed in Affine session that models channel interruption.
We utilize this idea and refine it as the proof term of explicit weakening rules. 
The refinement provides a better way to understand the channel cancellation
...
we provide a typed language where the exceptions are generalized. For instance,
we can simulate server timeout and client reconnect example studied by Brun and Dardha \cite{Brun2023}.

Adjoint logic generalizes the usual
binary session types by supporting multiple modes of communication \cite{Klaas2019}.
...
In other words, how to pass exceptions from one mode to another mode while both having the correct intended behavior and
keeping the safety of the language. Meanwhile, previous works model data layout using 
semi-axiomatic sequent calculus \cite{DeYoung2023}. Memory access errors such as 
writing to a freed memory cell or dereferencing an invalid address. In the future work,
